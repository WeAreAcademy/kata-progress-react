...
export function KataProgressApp() {
    ...
    useEffect(() => {
        async function fetchAndSaveKatas() {
            ...
            const progressRes = await fetch(`http://localhost:4000/user/${user.id}/kata_progress`);
    ...
    async function updateStatusOnKata(user_id: string, kata_id: string, newStatus: boolean) {
        ...
        try {
            const res = await axios.post(`http://localhost:4000/user/${user_id}/kata/${kata_id}/progress`, {
                user_id: user_id,
                kata_id: kata_id,
                is_done: newStatus
            })
                    ...
                    <Tbody>
                        {decoratedKatasToShow.map(k => (
                            <Tr key={k.kata.id}>
                                ...
                                <Td>
                                    <Checkbox
                                        onChange={(e) => updateStatusOnKata(user.id, k.kata.id, e.target.checked)}
