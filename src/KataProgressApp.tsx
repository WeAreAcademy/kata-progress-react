...
export function KataProgressApp({ user, isFaculty }: KataProgressAppProps) {
    ...
    const [katasProgressData, setKatasProgressData] = useState<
        KataProgressData[]
    >([]);
    const [searchTerm, setSearchTerm] = useState("");
    const [users, setUsers] = useState<SimpleUserWithCounts[]>([]);
    const [selectedUser, setSelectedUser] =
        useState<SimpleUserWithCounts | null>(null);
    ...
    useEffect(() => {
        async function fetchAndSaveUsers() {
            ...
            const usersData = (
                await axios.get(`${apiBaseURL}/users_with_counts`, { headers })
            ).data;
    ...
    useEffect(() => {
        async function fetchAndSaveKatasAndTheirProgress() {
            ...
            const progressRes = await axios.get(
                `${apiBaseURL}/user/${uidToFetch}/kata_progress`,
                { headers }
            );
    ...
    function calcDecoratedKatas() {
        return katas.map((k) => {
            const progress = katasProgressData.find(
                (pd) => pd.kata_id === k.id
            );
    ...
    function changeKataStatusLocallyWithDBReturnedValue(
        newRowFromDB: KataProgressData
    ): void {
        const isTarget = (kp: KataProgressData) =>
            kp.kata_id === newRowFromDB.kata_id &&
            kp.user_id === newRowFromDB.user_id;
        setKatasProgressData((prev) => {
            if (prev.find(isTarget)) {
                return [...prev].map((kp) =>
                    isTarget(kp) ? newRowFromDB : kp
                );
            } else {
    ...
    async function updateStatusOnKata(
        user_id: string,
        kata_id: string,
        statusChange: IStatusChange
    ) {
        console.log(
            `would update status on kata ${kata_id} for user: ${user_id}`,
            { statusChange }
        );
        const headers = await createAuthHeaders(user);
        ...
        try {
            ...
            if (res.status >= 200 && res.status < 400) {
                ...
                const returnedKataProgressDataRow: KataProgressData =
                    res.data.data;
                changeKataStatusLocallyWithDBReturnedValue(
                    returnedKataProgressDataRow
                );
                toast({
            ...
            toast({
                ...
                description:
                    "Error communicating with server - Please retry later",
                status: "error",
                duration: 2000,
                isClosable: true,
            });
    ...
    const decoratedKatasToShow = sortDecoratedKatas(
        decoratedKatas
            .filter((k) => k.kata.status !== "not ready")
            .filter((k) =>
                k.kata.name.toLowerCase().includes(searchTerm.toLowerCase())
            )
    );
    const countOfDoneKatas = decoratedKatas.filter(
        (dk) => dk.progress?.is_done
    ).length;
